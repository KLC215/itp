<template>
	<div>
		<div id="explain" class="well">
			<h4>While condition(s) is true, run the block of statement(s).</h4>
		</div>
		<div id="syntax">
			<button class="btn btn-info" @click="startWithSyntaxIntro">See syntax intro</button>
			<codemirror v-model="code[0]" :options="editorOptions" class="CodeMirror"></codemirror>
		</div>
		<hr>
		<div id="example">
			<button class="btn btn-info" @click="startWithExampleIntro">See example intro</button>
			<codemirror v-model="code[1]" :options="editorOptions" class="CodeMirror"></codemirror>
		</div>
		<hr>
			...
			return {
				...
				introConfig: {
					steps: [
						...
						{
							element: '#syntax',
							intro: "<h3><ul><li>Line 1: we will put the condition inside the brackets</li><li>Line 2: we will run our code if the condition(s) is true</li></ul></h3>"
						},
						{
