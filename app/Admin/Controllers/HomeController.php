...
class HomeController extends Controller
{
	public function index()
	{
		return Admin::content(function (Content $content) {
			...
			$content->header('Loop Administrate Dashboard');
			$content->description('manage all loop data');
			
			$content->row(function ($row) {
				$row->column(3, new InfoBox('Users', 'users', 'aqua', '/admin/game/users', User::all()->count()));
				$row->column(3, new InfoBox('Badges', 'certificate', 'green', '/admin/game/badges', Badge::all()->count()));
				$row->column(3, new InfoBox('Stages', 'stack-overflow', 'red', '/admin/game/stages', Stage::all()->count()));
				$row->column(3, new InfoBox('Questions', 'question', 'yellow', '/admin/game/questions', Question::all()->count()));
			});
			...
			$headers = ['Id', 'Name', 'Method', 'Path', 'IP address', 'Input', 'Created At'];
			
			$adminLogs = DB::table('admin_operation_log')->select()->get()->toArray();
			...
			$content->row((new Box('Admin Operation Log', new Table($headers, $adminLogs)))->style('info')->solid());
		});
	}
}
